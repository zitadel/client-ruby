...
- Generator: `ruby-plus`, run from the `openapi-generator-plus:enhanced` image.
...
Hand-written here: the bespoke authenticators under `lib/zitadel/client/auth/`, their tests, the
...
devbox run -- make test       # bundle exec rake test
...
the trace-context tests). This repo keeps its own `Gemfile + the gemspec`, so the generator cannot add
