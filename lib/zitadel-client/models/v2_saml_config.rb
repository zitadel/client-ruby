...
module ZitadelClient
  class V2SAMLConfig
    ...
    attr_accessor :with_signed_request
    ...
    def self.openapi_types
      {
        :'metadata_xml' => :'String',
        :'binding' => :'V2SAMLBinding',
        :'with_signed_request' => :'Boolean',
        :'name_id_format' => :'V2SAMLNameIDFormat',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ZitadelClient::V2SAMLConfig` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ZitadelClient::V2SAMLConfig`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'metadata_xml')
        self.metadata_xml = attributes[:'metadata_xml']
      end
      ...
      if attributes.key?(:'with_signed_request')
        self.with_signed_request = attributes[:'with_signed_request']
      end
    ...
    def list_invalid_properties
      ...
      if !@metadata_xml.nil? && @metadata_xml !~ pattern
        invalid_properties.push("invalid value for \"metadata_xml\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if !@metadata_xml.nil? && @metadata_xml !~ Regexp.new(/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] metadata_xml Value to be assigned
    def metadata_xml=(metadata_xml)
      if metadata_xml.nil?
        fail ArgumentError, 'metadata_xml cannot be nil'
      end

      pattern = Regexp.new(/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/)
      if metadata_xml !~ pattern
        fail ArgumentError, "invalid value for \"metadata_xml\", must conform to the pattern #{pattern}."
      end
