    ...
    class OIDCServiceAuthorizationError
      ...
      def initialize(attributes = {})
        unless attributes.is_a?(Hash)
          ...
          raise ArgumentError,
                'The input argument (attributes) must be a hash in `ZitadelClient::Model::OIDCServiceAuthorizationError` initialize method'
        end
        ...
        attributes = attributes.each_with_object({}) do |(k, v), h|
          unless acceptable_attribute_map.key?(k.to_sym)
            # MODIFIED: Updated class name in error message
            raise ArgumentError,
                  "`#{k}` is not a valid attribute in `ZitadelClient::Model::OIDCServiceAuthorizationError`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end

          h[k.to_sym] = v
        end

        self.error = if attributes.key?(:error)
                       attributes[:error]
                     else
