    ...
    class OIDCServiceSession
      ...
      def initialize(attributes = {})
        unless attributes.is_a?(Hash)
          ...
          raise ArgumentError,
                'The input argument (attributes) must be a hash in `ZitadelClient::Model::OIDCServiceSession` initialize method'
        end
        ...
        attributes = attributes.each_with_object({}) do |(k, v), h|
          unless acceptable_attribute_map.key?(k.to_sym)
            # MODIFIED: Updated class name in error message
            raise ArgumentError,
                  "`#{k}` is not a valid attribute in `ZitadelClient::Model::OIDCServiceSession`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end

          h[k.to_sym] = v
        end

        self.session_id = attributes[:session_id] if attributes.key?(:session_id)

        return unless attributes.key?(:session_token)
      ...
      def list_invalid_properties
        ...
        if !@session_token.nil? && @session_token.to_s.empty?
      ...
      def valid?
        ...
        return false if !@session_id.nil? && @session_id.to_s.empty?
        return false if !@session_token.nil? && @session_token.to_s.length > 200
        return false if !@session_token.nil? && @session_token.to_s.empty?
      ...
      def session_id=(session_id)
        ...
        if session_id.to_s.length > 200
          raise ArgumentError, 'invalid value for "session_id", the character length must be smaller than or equal to 200.'
        end

        if session_id.to_s.empty?
          raise ArgumentError, 'invalid value for "session_id", the character length must be great than or equal to 1.'
        end
      ...
      def session_token=(session_token)
        ...
        if session_token.to_s.length > 200
          raise ArgumentError, 'invalid value for "session_token", the character length must be smaller than or equal to 200.'
        end

        if session_token.to_s.empty?
          raise ArgumentError, 'invalid value for "session_token", the character length must be great than or equal to 1.'
        end
