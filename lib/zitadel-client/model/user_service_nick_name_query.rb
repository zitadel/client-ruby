  ...
  class UserServiceNickNameQuery
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `ZitadelClient::Model::UserServiceNickNameQuery` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `ZitadelClient::Model::UserServiceNickNameQuery`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @nick_name.nil?
        invalid_properties.push('invalid value for "nick_name", nick_name cannot be nil.')
      end
      ...
      if @nick_name.to_s.length < 1
        invalid_properties.push('invalid value for "nick_name", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @nick_name.nil?
      return false if @nick_name.to_s.length > 200
      return false if @nick_name.to_s.length < 1
      true
    end
    ...
    def nick_name=(nick_name)
      ...
      if nick_name.to_s.length < 1
        fail ArgumentError, 'invalid value for "nick_name", the character length must be great than or equal to 1.'
      end
