...
module ZitadelClient
  ...
  module Model
    class IdentityProviderServiceIDPConfig
      attr_accessor :options, :ldap, :google, :oauth, :oidc, :jwt, :github, :github_es, :gitlab, :gitlab_self_hosted,
                    :azure_ad, :apple, :saml
      ...
      def initialize(attributes = {})
        unless attributes.is_a?(Hash)
          ...
          raise ArgumentError,
                'The input argument (attributes) must be a hash in `ZitadelClient::Model::IdentityProviderServiceIDPConfig` initialize method'
        end
        ...
        attributes = attributes.each_with_object({}) do |(k, v), h|
          unless acceptable_attribute_map.key?(k.to_sym)
            # MODIFIED: Updated class name in error message
            raise ArgumentError,
                  "`#{k}` is not a valid attribute in `ZitadelClient::Model::IdentityProviderServiceIDPConfig`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
        ...
        self.ldap = attributes[:ldap] if attributes.key?(:ldap)

        self.google = attributes[:google] if attributes.key?(:google)

        self.oauth = attributes[:oauth] if attributes.key?(:oauth)

        self.oidc = attributes[:oidc] if attributes.key?(:oidc)

        self.jwt = attributes[:jwt] if attributes.key?(:jwt)

        self.github = attributes[:github] if attributes.key?(:github)

        self.github_es = attributes[:github_es] if attributes.key?(:github_es)

        self.gitlab = attributes[:gitlab] if attributes.key?(:gitlab)

        self.gitlab_self_hosted = attributes[:gitlab_self_hosted] if attributes.key?(:gitlab_self_hosted)

        self.azure_ad = attributes[:azure_ad] if attributes.key?(:azure_ad)

        self.apple = attributes[:apple] if attributes.key?(:apple)

        return unless attributes.key?(:saml)
      ...
      def ==(other)
        ...
        self.class == other.class &&
          ...
          jwt == other.jwt &&
          github == other.github &&
          github_es == other.github_es &&
          gitlab == other.gitlab &&
          gitlab_self_hosted == other.gitlab_self_hosted &&
          azure_ad == other.azure_ad &&
          apple == other.apple &&
          saml == other.saml
      end
