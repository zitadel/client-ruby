    ...
    class OIDCServiceAuthRequest
      ...
      def initialize(attributes = {})
        unless attributes.is_a?(Hash)
          ...
          raise ArgumentError,
                'The input argument (attributes) must be a hash in `ZitadelClient::Model::OIDCServiceAuthRequest` initialize method'
        end
        ...
        attributes = attributes.each_with_object({}) do |(k, v), h|
          unless acceptable_attribute_map.key?(k.to_sym)
            # MODIFIED: Updated class name in error message
            raise ArgumentError,
                  "`#{k}` is not a valid attribute in `ZitadelClient::Model::OIDCServiceAuthRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
        ...
        self.creation_date = attributes[:creation_date] if attributes.key?(:creation_date)

        self.client_id = attributes[:client_id] if attributes.key?(:client_id)

        if attributes.key?(:scope) && (value = attributes[:scope]).is_a?(Array)
          self.scope = value
        end

        self.redirect_uri = attributes[:redirect_uri] if attributes.key?(:redirect_uri)

        if attributes.key?(:prompt) && (value = attributes[:prompt]).is_a?(Array)
          self.prompt = value
        end

        if attributes.key?(:ui_locales) && (value = attributes[:ui_locales]).is_a?(Array)
          self.ui_locales = value
        end

        self.login_hint = attributes[:login_hint] if attributes.key?(:login_hint)

        self.max_age = attributes[:max_age] if attributes.key?(:max_age)

        return unless attributes.key?(:hint_user_id)
      ...
      def ==(other)
        ...
        self.class == other.class &&
          ...
          prompt == other.prompt &&
          ui_locales == other.ui_locales &&
          login_hint == other.login_hint &&
          max_age == other.max_age &&
          hint_user_id == other.hint_user_id
      end
