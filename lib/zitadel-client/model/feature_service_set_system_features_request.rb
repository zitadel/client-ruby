    ...
    class FeatureServiceSetSystemFeaturesRequest
      ...
      def initialize(attributes = {})
        unless attributes.is_a?(Hash)
          ...
          raise ArgumentError,
                'The input argument (attributes) must be a hash in `ZitadelClient::Model::FeatureServiceSetSystemFeaturesRequest` initialize method'
        end
        ...
        attributes = attributes.each_with_object({}) do |(k, v), h|
          unless acceptable_attribute_map.key?(k.to_sym)
            # MODIFIED: Updated class name in error message
            raise ArgumentError,
                  "`#{k}` is not a valid attribute in `ZitadelClient::Model::FeatureServiceSetSystemFeaturesRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
      ...
      def hash
        [login_default_org, oidc_trigger_introspection_projections, oidc_legacy_introspection, user_schema, oidc_token_exchange,
         actions, improved_performance, oidc_single_v1_session_termination, disable_user_token_event, enable_back_channel_logout, login_v2, permission_check_v2].hash
      end
