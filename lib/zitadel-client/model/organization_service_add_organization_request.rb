    ...
    class OrganizationServiceAddOrganizationRequest
      ...
      def initialize(attributes = {})
        unless attributes.is_a?(Hash)
          ...
          raise ArgumentError,
                'The input argument (attributes) must be a hash in `ZitadelClient::Model::OrganizationServiceAddOrganizationRequest` initialize method'
        end
        ...
        attributes = attributes.each_with_object({}) do |(k, v), h|
          unless acceptable_attribute_map.key?(k.to_sym)
            # MODIFIED: Updated class name in error message
            raise ArgumentError,
                  "`#{k}` is not a valid attribute in `ZitadelClient::Model::OrganizationServiceAddOrganizationRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
        ...
        return unless attributes.key?(:admins)
      ...
      def name=(name)
        ...
        if name.to_s.length > 200
          raise ArgumentError, 'invalid value for "name", the character length must be smaller than or equal to 200.'
        end

        if name.to_s.empty?
          raise ArgumentError, 'invalid value for "name", the character length must be great than or equal to 1.'
        end
