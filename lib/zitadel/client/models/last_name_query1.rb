  ...
  class LastNameQuery1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::LastNameQuery1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::LastNameQuery1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @last_name_query.nil?
        invalid_properties.push('invalid value for "last_name_query", last_name_query cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @last_name_query.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [BetaUserServiceLastNameQuery] last_name_query Value to be assigned
    def last_name_query=(last_name_query)
      if last_name_query.nil?
        fail ArgumentError, 'last_name_query cannot be nil'
      end
