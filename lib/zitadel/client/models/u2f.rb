  ...
  class U2f
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::U2f` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::U2f`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @u2f.nil?
        invalid_properties.push('invalid value for "u2f", u2f cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @u2f.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [UserServiceAuthFactorU2F] u2f Value to be assigned
    def u2f=(u2f)
      if u2f.nil?
        fail ArgumentError, 'u2f cannot be nil'
      end
