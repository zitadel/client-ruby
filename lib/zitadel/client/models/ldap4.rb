  ...
  class Ldap4
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::Ldap4` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::Ldap4`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ldap.nil?
        invalid_properties.push('invalid value for "ldap", ldap cannot be nil.')
      end
    ...
    # @param [BetaUserServiceIDPLDAPAccessInformation] ldap Value to be assigned
    def ldap=(ldap)
      if ldap.nil?
        fail ArgumentError, 'ldap cannot be nil'
      end
