...
module Zitadel::Client::Models
    ...
    attr_accessor :is_api

    attr_accessor :is_oidc
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::BetaAppServiceRegenerateClientSecretRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::BetaAppServiceRegenerateClientSecretRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_api')
        self.is_api = attributes[:'is_api']
      end

      if attributes.key?(:'is_oidc')
        self.is_oidc = attributes[:'is_oidc']
      end
    end
    ...
    def hash
      [project_id, application_id, is_api, is_oidc].hash
    end
