  ...
  class IdpIntent1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::IdpIntent1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::IdpIntent1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @idp_intent.nil?
        invalid_properties.push('invalid value for "idp_intent", idp_intent cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @idp_intent.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [BetaUserServiceIDPIntent] idp_intent Value to be assigned
    def idp_intent=(idp_intent)
      if idp_intent.nil?
        fail ArgumentError, 'idp_intent cannot be nil'
      end

      @idp_intent = idp_intent
    end
