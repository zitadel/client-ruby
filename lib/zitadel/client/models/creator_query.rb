  ...
  class CreatorQuery
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::CreatorQuery` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::CreatorQuery`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'creator_query')
        self.creator_query = attributes[:'creator_query']
      else
        self.creator_query = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @creator_query.nil?
        invalid_properties.push('invalid value for "creator_query", creator_query cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @creator_query.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [SessionServiceCreatorQuery] creator_query Value to be assigned
    def creator_query=(creator_query)
      if creator_query.nil?
        fail ArgumentError, 'creator_query cannot be nil'
      end
