...
module Zitadel::Client::Models
  class ActionServiceBetaTargetNameFilter
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::ActionServiceBetaTargetNameFilter` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::ActionServiceBetaTargetNameFilter`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'method')
        ...
        self.method = 'TEXT_FILTER_METHOD_EQUALS'
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@target_name.nil? && @target_name.to_s.length > 200
        invalid_properties.push('invalid value for "target_name", the character length must be smaller than or equal to 200.')
      end
    ...
    def valid?
      ...
      return false if !@target_name.nil? && @target_name.to_s.length > 200
      true
    end

    # Custom attribute writer method with validation
    # @param [String] target_name Value to be assigned
    def target_name=(target_name)
      if target_name.nil?
        fail ArgumentError, 'target_name cannot be nil'
      end

      if target_name.to_s.length > 200
        fail ArgumentError, 'invalid value for "target_name", the character length must be smaller than or equal to 200.'
      end
