...
module Zitadel::Client::Models
    ...
    def self.openapi_types
      {
        :'user_id' => :'String',
        :'auth_factors' => :'Array<UserServiceAuthFactors>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::UserServiceListAuthenticationFactorsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::UserServiceListAuthenticationFactorsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'auth_factors')
        if (value = attributes[:'auth_factors']).is_a?(Array)
          self.auth_factors = value
        end
      end
    ...
    def hash
      [user_id, auth_factors, states].hash
    end
