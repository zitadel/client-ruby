  ...
  class GithubEs
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::GithubEs` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::GithubEs`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'github_es')
        self.github_es = attributes[:'github_es']
      else
        self.github_es = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @github_es.nil?
        invalid_properties.push('invalid value for "github_es", github_es cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @github_es.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [IdentityProviderServiceGitHubEnterpriseServerConfig] github_es Value to be assigned
    def github_es=(github_es)
      if github_es.nil?
        fail ArgumentError, 'github_es cannot be nil'
      end
