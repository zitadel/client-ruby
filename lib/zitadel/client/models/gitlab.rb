  ...
  class Gitlab
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::Gitlab` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::Gitlab`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @gitlab.nil?
        invalid_properties.push('invalid value for "gitlab", gitlab cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @gitlab.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [IdentityProviderServiceGitLabConfig] gitlab Value to be assigned
    def gitlab=(gitlab)
      if gitlab.nil?
        fail ArgumentError, 'gitlab cannot be nil'
      end
