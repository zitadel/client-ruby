  ...
  class GitlabSelfHosted
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::GitlabSelfHosted` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::GitlabSelfHosted`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'gitlab_self_hosted')
        self.gitlab_self_hosted = attributes[:'gitlab_self_hosted']
      else
        self.gitlab_self_hosted = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @gitlab_self_hosted.nil?
        invalid_properties.push('invalid value for "gitlab_self_hosted", gitlab_self_hosted cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @gitlab_self_hosted.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [IdentityProviderServiceGitLabSelfHostedConfig] gitlab_self_hosted Value to be assigned
    def gitlab_self_hosted=(gitlab_self_hosted)
      if gitlab_self_hosted.nil?
        fail ArgumentError, 'gitlab_self_hosted cannot be nil'
      end
