  ...
  class InUserEmailsQuery1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        ...
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zitadel::Client::Models::InUserEmailsQuery1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          # MODIFIED: Updated class name in error message
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zitadel::Client::Models::InUserEmailsQuery1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @in_user_emails_query.nil?
        invalid_properties.push('invalid value for "in_user_emails_query", in_user_emails_query cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @in_user_emails_query.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [BetaUserServiceInUserEmailsQuery] in_user_emails_query Value to be assigned
    def in_user_emails_query=(in_user_emails_query)
      if in_user_emails_query.nil?
        fail ArgumentError, 'in_user_emails_query cannot be nil'
      end

      @in_user_emails_query = in_user_emails_query
    end
