...
module Zitadel::Client::Api
  ...
  @api_client = api_client
    ...
    return data
      ...
      # @param [Hash] opts the optional parameters
    # @return [NoOp200Response8]
    def no_op(opts = {})
    if @api_client.config.debugging
    @api_client.config.logger.debug 'Calling API: Api::OIDCServiceApi.no_op ...' # MODIFIED
    end
    # resource path
    local_var_path = '/6a6432fe'
    ...
    # return_type
    return_type = opts[:debug_return_type] || 'NoOp200Response8'

    # auth_names
    auth_names = opts[:debug_auth_names] || ['zitadelAccessToken']

    new_options = opts.merge(
    :operation => :"Api::OIDCServiceApi.no_op", # MODIFIED
    :header_params => header_params,
    :query_params => query_params,
    :form_params => form_params,
    :body => post_body,
    ...
    if @api_client.config.debugging
    @api_client.config.logger.debug "API called: Api::OIDCServiceApi#no_op\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}" # MODIFIED
    end
    return data
    end
  end
